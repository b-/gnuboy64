...
struct mbc __attribute__ ((aligned (16))) mbc;
struct rom __attribute__ ((aligned (16))) rom;
struct ram __attribute__ ((aligned (16))) ram;
...
byte mem_read(int a)
{
	...
	switch (ha)
	{
	case 0x0: return rom.bank[0][a];
	case 0x1:return 0xff; /* not reached */
	case 0x2:
		return rom.bank[0][a];
	case 0x4:
	case 0x6:
...
#if 0
/*New faster implementations for HwDMA operations ~conle*/
#include "../umem/umem.h"
...
void mem_write_range(int  a,int cnt,const byte* in) {
	int n,i;
	byte ha = (a>>12) & 0xE;
...
void mem_read_range(int a,int cnt,byte* out)
{
	int n,i;
	byte ha = (a>>12) & 0xE;
